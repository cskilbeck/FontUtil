            ...
            if (imageTable.Count > layerIndex)
            {
                ...
                if (i.pageIndex >= 0 && i.pageIndex < font.pages.Count)
                {
                    ...
                    using (Graphics d = Graphics.FromImage(dest))
                    {
                        ...
                        d.SmoothingMode = SmoothingMode.None;
                        d.InterpolationMode = InterpolationMode.NearestNeighbor;
                        ...
                        float x = (float)Math.Floor(pos.X + i.drawOffset.X + rc + (float)layer.offset.X);
                        float y = (float)Math.Floor(pos.Y + i.drawOffset.Y + (float)layer.offset.Y);
